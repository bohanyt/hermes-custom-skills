#!/usr/bin/env python3
"""
topic_chunker.py v2 — Hybrid chunking + ratio-based topic labeling untuk transkrip livestream
...
Scoring:
  ...
  - storytime_ratio = pos_hits / (pos_hits + neg_hits)
...
VIDEO_TYPES = {
    ...
    "storytime": {
        ...
        "keywords": [
            ...
            "kamar", "gua mau", "aku mau", "gua lagi", "aku lagi",
            "mikirnya", "rasanya",
            "jadi gini", "dengerin dulu", "ini ceritanya",
            "masalahnya", "intinya", "ceritanya",
        ],
        "counter_keywords": [
...
def score_chunk(chunk):
    ...
    for vtype, vdef in VIDEO_TYPES.items():
        ...
        ratio = pos_hits / total_hits if total_hits > 0 else 0.0
        dur = chunk.duration
...
def label_chunk(chunk):
    ...
    for vtype, scores in sorted_types:
        ...
        if scores["pos_hits"] >= min_hits and scores["ratio"] >= min_ratio and scores["total_hits"] >= 3:
            best_type = vtype
...
def export_results(chunks, output_path, srt_path=""):
    result = {"source": srt_path, "total_chunks": len(chunks), "chunks": []}
    for c in chunks:
        entry = {
            "chunk_id": c.chunk_id,
            "start_ts": c.start_ts,
...
def print_summary(chunks):
    ...
    for c in chunks:
        ...
        if c.label_signals:
            print(f"     Signals: {', '.join(c.label_signals[:8])}")
        print(f"     {c.text[:150]}...")
    print(f"\n{'='*70}")
